...
import { propTypesUser } from './Config';
...
const User = React.createClass({
  ...
  render() {
    let user = this.props.users.find((u) => u.id === this.props.userNoteId);
    if (!user) {
      return null;
    }

    const noteList = this.state.note.filter((note) => note.user_id === user.id).map((note, id) => (
    <div key={ id }>
          <li onClick={ this.setNoteShow.bind(null, id)}>
            { note.day }
          </li>
          <div className="notess">
            <div className="mo">心情：<img src={ moodImgSrc(note.mood)} alt=""/></div>
      ...
      dangerouslySetInnerHTML={ {
        __html: marked(note.content)
            ...
            <div className="comment">
              ...
              <div className="all_com" style={ cssDisplay(this.state.noteShow === id)}>暂无评论</div>
            </div>
            <div className="add_comment"><a onClick={ this.addCom }>添加评论</a></div>
            <div className="add_ssuccess" style={ cssDisplay(this.state.addComSuccess)}>添加成功</div>
            <div id="mask" onClick={ this.closeAdd } style={ cssDisplay(this.state.addComShow)}></div>
            <div className="add_com_text" style={ cssDisplay(this.state.addComShow)}>
              <div className="title">
                我的评论
              </div>
